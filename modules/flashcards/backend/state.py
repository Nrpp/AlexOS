"""Real, persisted flashcards - front + back. Same generic module-data
pattern as modules/shopping_list (duplicated, not shared - modules are
...
async def create_item(storage: StorageManager, front: str, back: str) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "front": front, "back": back}
    items.append(item)
