"""Real, persisted cheat sheet - title + content. Same generic
...
async def create_item(storage: StorageManager, title: str, content: str) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "title": title, "content": content}
    items.append(item)
