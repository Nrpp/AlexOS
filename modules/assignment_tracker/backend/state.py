"""Real, persisted assignment tracker - title + dueDate + done. Same
...
MODULE_NAME = "assignment_tracker"
_LIST_KEY = "items"
...
async def create_item(storage: StorageManager, title: str, due_date: str) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "title": title, "dueDate": due_date, "done": False}
    items.append(item)
