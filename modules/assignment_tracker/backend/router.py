...
from .state import clear_completed, create_item, delete_item, list_items, toggle_item
...
async def post_item(body: CreateItemRequest, request: Request) -> dict:
    return await create_item(request.app.state.storage_manager, body.title, body.dueDate)


@router.patch("/items/{item_id}")
async def patch_item(item_id: str, body: ToggleItemRequest, request: Request) -> dict:
    item = await toggle_item(request.app.state.storage_manager, item_id, body.done)
    if item is None:
...
@router.post("/items/clear-completed", response_model=None)
async def post_clear_completed(request: Request) -> None:
    await clear_completed(request.app.state.storage_manager)
