"""Real, persisted vocabulary builder - word + definition. Same generic
...
MODULE_NAME = "vocabulary_builder"
_LIST_KEY = "items"
...
async def create_item(storage: StorageManager, word: str, definition: str) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "word": word, "definition": definition}
    items.append(item)
