"""Real, persisted grade tracker - title + score + maxScore. Same
...
async def create_item(storage: StorageManager, title: str, score: float, max_score: float) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "title": title, "score": score, "maxScore": max_score}
    items.append(item)
