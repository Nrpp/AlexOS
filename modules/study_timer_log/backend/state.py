"""Real, persisted study timer log - subject + minutes + date. Same
...
async def create_item(storage: StorageManager, subject: str, minutes: int, date: str) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "subject": subject, "minutes": minutes, "date": date}
    items.append(item)
