"""Real, persisted course list - name + professor + credits. Same
...
async def create_item(storage: StorageManager, name: str, professor: str, credits: float) -> dict[str, Any]:
    items = await list_items(storage)
    item = {"id": str(uuid.uuid4()), "name": name, "professor": professor, "credits": credits}
    items.append(item)
