...
modules/<name>/
  ...
  README.md
  backend/
    __init__.py       # exposes `router` and/or `on_load(event_bus)`
    ...
  frontend/
...
1. Mounts `router` (a FastAPI `APIRouter`, if the module defines one) at
   `/api/v1/modules/<name>`.
2. Calls `on_load(event_bus)` (if defined) - the only way a module gets a
   handle to the Event Bus. This is where a module starts background
   work, like `clock`'s one-second tick loop.
...
exports a component accepting `{ eventBus?: EventBusLike }` - the same
dependency-injection pattern as the backend's `on_load`, so a widget never
imports the host app's context directly and stays portable to a future
...
4. If the module has a widget, export a default component from
